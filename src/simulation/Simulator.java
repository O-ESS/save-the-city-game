...
public class Simulator implements WorldListener {
	...
	private ArrayList<Boolean> deadpeople;
		...
		for (int i = 0; i < buildings.size(); i++) {
			...
			for (int j = 0; j < citizens.size(); j++) {
				...
				if (citizen.getLocation() == building.getLocation()) {
					...
					// emergencyService.changeIcon(citizen.getLocation().getX(),citizen.getLocation().getY(),"peopleAndBuilding");
				}
			}
		}
		deadpeople=new ArrayList<Boolean>();
		 for(int i=0;i<citizens.size();i++)
			 deadpeople.add(false);
	...
	private void loadBuildings(String path) throws Exception {
		...
		while (line != null) {
			...
			buildings.add(b);
			// emergencyService.changeIcon(x, y, "building");
			line = br.readLine();
		}
		br.close();
	}
	...
	private void loadCitizens(String path) throws Exception {
		...
		while (line != null) {
			...
			citizens.add(c);
			// emergencyService.changeIcon(x, y, "people");
			line = br.readLine();
		}
		br.close();
	}
	...
	public void nextCycle() throws Exception {
		...
		for (int i = 0; i < plannedDisasters.size(); i++) {
			...
			if (d.getStartCycle() == currentCycle) {
				...
				log += " Location: " + d.getTarget().getLocation().getX() + " "
		...
		for (int i = 0; i < citizens.size(); i++) {
			...
			if(citizens.get(i).getState()==CitizenState.DECEASED && !deadpeople.get(i)){
				deadpeople.set(i, true);
				deads+="Citizen: "+citizens.get(i).getName()+" has died in location: "+citizens.get(i).getLocation()+"\n";
			}
		}
 
		//int numOfCasualities = calculateCasualties();
		// emergencyService.changeview(currentCycle, numOfCasualities);
		if(deads.length()!=0)
			log+=deads;
		log+="-------------------------------------------------------------------------------------------------------------------------------\n";
	}
	...
	public void setSOSListener(SOSListener e) {
		emergencyService = e;
	}
	...
	// public void getInformation(int i,int j){
	...
	// }
	// }
	// emergencyService.changeInfo("Location: "+i+" "+j);
	// return;
	// }
	...
	public String getview() {
		String display = "";
		display += "CurrentCycle: " + currentCycle + "\n" + "number of casualites: "
				+ calculateCasualties();
		return display;
	}
